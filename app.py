...
# ==========================================
    ...
    if st.session_state.page_mode == 'home':
        ...
        with st.form("editor"):
            ...
            st.markdown("---")
            st.write("#### 💰 販売価格の設定")
            price = st.number_input("ダウンロード価格 (円)", min_value=980, max_value=98000, value=980, step=100, help="980円〜98,000円の間で設定できます。")
            
            c1, c2 = st.columns(2); sub_free = c1.form_submit_button("🌐 無料公開", type="primary"); is_pub = c2.checkbox("ポータル掲載"); sub_paid = c2.form_submit_button(f"💾 {price}円で購入設定")
            ...
            if sub_free or sub_paid:
                if not email: st.error("Email必須")
                    ...
                    try:
                        ...
                        if sub_paid:
                            # Stripeに設定した価格(price)を渡す
                            sess = stripe.checkout.Session.create(
                                payment_method_types=['card'], 
                                line_items=[{'price_data':{'currency':'jpy','product_data':{'name':f'診断データ: {main_heading}'},'unit_amount':price},'quantity':1}], 
                                mode='payment', 
