...
with st.sidebar:
    st.header("🧠 AI設定")
    # GitHubで公開するため、APIキーは入力式（またはSecrets）にします
    api_key_input = st.text_input("OpenAI APIキー (sk-...)", type="password")
    
    # Streamlit Secretsからキーを取得（設定されている場合）
    if not api_key_input and "OPENAI_API_KEY" in st.secrets:
        api_key = st.secrets["OPENAI_API_KEY"]
        st.success("✅ APIキーがシステム設定から読み込まれました")
    else:
    ...
    if st.button("AIで構成案を作る", type="primary"):
        if not api_key:
            ...
            try:
                with st.spinner("AIが思考中... (15〜30秒ほどお待ちください)"):
                    ...
                    {{
                        ...
                        "questions": [
                            {{
                                "question": "質問文",
                                "answers": [
                                    {{ "text": "選択肢1(A向け)", "type": "A" }},
                                    {{ "text": "選択肢2(B向け)", "type": "B" }},
                                    {{ "text": "選択肢3(C向け)", "type": "C" }},
                                    {{ "text": "選択肢4(A向け)", "type": "A" }}
                                ]
                            }}
                        ]
                    }}
                    ...
                    st.success("AI生成完了！フォームに反映されました。")
                    st.rerun()
